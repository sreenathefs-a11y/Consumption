...
const readOnlyClient=new ApiClient(()=>({mode:'remote',baseUrl:'https://script.google.com/macros/s/deployment/exec',apiToken:''}),okFetch);
...
const writeClient=new ApiClient(()=>({mode:'remote',baseUrl:'https://script.google.com/macros/s/deployment/exec',apiToken:'valid-write-token'}),okFetch);
...
const noWriteClient=new ApiClient(()=>({mode:'remote',baseUrl:'https://script.google.com/macros/s/deployment/exec',apiToken:''}),async()=>{fetchCount++;return okFetch()});await assert.rejects(()=>noWriteClient.post('updateCase',{}),error=>error.code==='WRITE_ACCESS_REQUIRED');assert.equal(fetchCount,0);assert.equal(writeEvent,true);
